...
TEXT;
$config['css']            = <<<'TEXT'
pre   {background-color: #f9f9f9; border: 1px solid #D0D0D0; overflow-x: auto; white-space: pre-wrap; word-wrap: break-word; display: block; margin: 1em 0; padding: .6em;}
...
TEXT;
$config['pre_content'] = function() {
    $title  = settings('content_title');
    $slogan = settings('content_slogan');
    $intro  = settings('content_intro');
    $ctt    =
        (!empty($title) ?
            '<h1>'.$title
            .(!empty($slogan) ? ' <small>'.$slogan.'</small>' : '')
            .'</h1>'
        : '')
        .(!empty($intro) ?
            '<p>'.$intro.'</p>'
        : '')
    ;
    return (!empty($ctt) ? '<div class="page-header">'.$ctt.'</div>' : '');
};
$config['404_content'] = function() {
    return <<<TEXT
...
// HTML5-quick-template internals
$config['hqt_settings']['brand_icon']     = '<i class="fa fa-book"></i>';
$config['hqt_settings']['navbar_items']   = array('menu', 'toc', 'top', 'bottom');
$config['hqt_settings']['profiler_mode']  = 'on';
$config['hqt_settings']['mask_list_item_content_toc'] = '<a href="%s">%s</a>';
$config['hqt_settings']['app_dependencies'] = array(
